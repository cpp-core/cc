...
public:
    ...
    using AdjancencyMap = std::map<string,strings>;
    ...
    size_t size(const string& id) const;

    // Return the size for the i'th source for <id>.
    size_t size(const string& id, int i) const;

    // Return the source <id>'s for the given <id>.
    strings sources(const string& id) const;

    // Return the i'th source <id> for the given <id>.
    string source(const string& id, int i) const;

    // Register <cursor> with the given <id>.
    void register_cursor(Cursor *cursor, const string& id);
    ...
    template<class P>
    void apply_barriers(P& processor, const string& id) {
...
private:
    ...
    Cursor *get_cursor(const string& id);
